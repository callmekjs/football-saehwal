 ...
 * 나왔다.
 *
 * 지금은 **경합 시간**에 건다. 우리가 공을 가졌고 상대 수비수가 경합
 * 거리 안에 붙어 있는 시간에 비례해 판정을 굴린다. 실제 축구의 반칙도
 * "붙어 있는 시간"에 비례한다.
 *
...
/**
 * 반칙 판정을 **굴리는** 거리(미터). 이 안에서만 난수를 뽑는다.
 *
 * ★ 이 값은 축구가 아니라 **재현성**이 정한다. 연출 난수는 뽑는 자리가
 * 하나만 바뀌어도 이후 수열이 통째로 밀려 같은 시드의 화면이 달라진다.
 * 그래서 뽑는 자리는 그대로 두고, 실제로 반칙이 되는지는 아래
 * `FOUL_CONTACT` 가 따로 본다.
 */
const FOUL_WATCH = 6
/**
 * 반칙이 **성립하는** 거리(미터).
 ...
 * 선수 간 거리가 중앙값 **3.80m**, 최대 **5.99m**였고 39회가 3미터
 * 밖이었다. 6미터 떨어진 선수는 상대를 넘어뜨릴 수도, 붙잡을 수도, 발을
 * 밟을 수도 없다. 접촉이 없는 곳에서 프리킥이 나오면 관전자는 무엇 때문에
 * 경기가 멈췄는지 알 수 없다.
 *
 * 2.4미터는 이 파일이 이미 **"붙어 있다"** 로 쓰는 거리다(`updatePressure`
 * 의 압박 게이지가 쌓이기 시작하는 지점). 발끝이 닿는 1.7미터보다 넓은
 * 것은 늦게 뻗은 발과 몸으로 막는 반칙이 그 사이에서 나오기 때문이다.
 */
const FOUL_CONTACT = 2.4
/**
 * 붙어 있는 1초에 반칙이 나는 비율.
 *
 * 접촉 거리를 6미터에서 2.4미터로 좁히면 판정 자리가 3.5%로 줄어든다.
 * 그만큼 올려 **하프당 상대 반칙 횟수를 그대로 지킨다.** 우리가 프리킥을
 * 얻는 장면 자체가 이 기능의 목적이므로, 거리를 고치면서 빈도까지 떨어뜨리면
 * 고치는 의미가 없다.
 */
const FOUL_RATE = 4.8
/**
 ...
 * 그 선수를 지나쳐 계속 흐른다.
 *
 * **구르는 공의 감속은 두 성분의 합이다.**
 *
 * 하나는 잔디를 눌러 생기는 **구름 저항**이다. 속도와 무관한 일정한
 * 감속이고 `a = μ·g` 다. 짧게 깎은 천연 잔디의 μ 는 대략 0.09라
 * `0.09 × 9.81 ≈ 0.9 m/s²` 다.
 *
 * 다른 하나는 **공기 저항**이고 이쪽이 훨씬 크다. 항력은 속도의
 * **제곱**에 비례한다 — `a = ½ρ·C_d·A/m · v²` 다. 5호 공(지름 22cm,
 * 430g)에 공기 밀도 1.2, 항력계수 0.25 를 넣으면 계수가 0.0135 다.
 * 초속 25미터에서 8.4 m/s², 걸음 속도에서는 0.03 m/s² 로 사실상 0 이다.
 *
 * ★ **이 항이 제곱이라는 것이 핵심이다.** 전에는 속도에 **비례**하는
 * 항으로 두었는데(`0.54 + 0.1·v`), 그러면 초속 20미터의 공이 1초에
 * 속력의 12%밖에 안 잃는다. 실제로는 31%를 잃는다. 그래서 세게 찬 공이
 * 화면에서 **거의 등속으로 굴러갔다** — 사용자가 "공이 계속 같은 속도로
 * 굴러간다"고 지적한 것이 이것이다. 느린 공은 원래도 맞게 죽고 있어서
 * 빠른 공에서만 어긋났고, 방향만 보는 검사는 그것을 못 잡았다.
 *
 * 고친 뒤 한 번 찬 공이 서기까지 가는 거리 — 초속 5미터 12m,
 * 15미터 55m, 25미터 87m 다. 실제 축구의 땅볼이 그렇다.
 */
const GRAVITY = 9.81
/** 잔디 구름 저항. 속도와 무관한 일정한 감속(m/s²) */
const ROLL_DECEL = 0.9
/** 공기 저항 계수. **속도의 제곱**에 곱한다 — ½ρ·C_d·A/m */
const ROLL_DRAG = 0.0135
/**
 * 튕김.
 *
 ...
 *
 * 4.99미터는 5미터나 앞선 명백한 위치를 온사이드로 놓치지 않으면서도
 * 점으로 그린 선수의 경계 흔들림을 거르는 최대 여유다. p02 간격 시드
 * 100하프를 실제 관전 속도로 재면 판당 0.95회다. 실제 축구를 이 15분
 * 구간으로 환산한 0.7~1.0회 안이다.
 *
...
export class VisualMatch {
  ...
  private foulMute = 0
  /**
   * 주심이 카드를 든 시뮬 틱. 없으면 -1.
   *
   * 같은 반칙에서 나온 프리킥이 그 카드 동작을 덮어쓰지 않게 하는 데만
   * 쓴다. 자세한 이유는 `syncFouls` 에 적었다.
   */
  private cardTick = -1
  /**
  ...
  private captureDowned(state: MatchState) {
    for (let i = this.downLogLen; i < state.log.length; i++) {
      ...
      if (e.kind === 'CARD' || e.kind === 'SEND_OFF') {
        ...
        if (who) {
          this.blowWhistle('CARD', who.x, who.y, e.kind === 'SEND_OFF')
          this.cardTick = e.tick
        }
      }
  ...
  private syncFouls(state: MatchState) {
    for (let i = this.lastLogLen; i < state.log.length; i++) {
      ...
      /**
       * ★ **같은 반칙에 경고가 붙었으면 주심은 카드를 든다.**
       *
       * `captureDowned` 가 먼저 카드 동작을 만드는데, 바로 뒤따라오는
       * 이 프리킥이 같은 자리에 반칙 신호를 덮어썼다. 선수 원 옆에는
       * 노란 배지가 붙는데 주심은 카드를 꺼내지 않아, 경기를 보는 사람은
       * 카드가 나온 줄을 모른다. 실측으로 **경고 41회 중 29회(70.7%)**가
       * 그렇게 사라졌다.
       *
       * 프리킥 자체가 없어지지는 않는다 — 공은 그대로 그 자리에 놓이고,
       * 안내판도 `calloutOf` 가 이미 카드를 프리킥보다 먼저 읽는다.
       * 여기서 지키는 것은 **주심의 손**뿐이다.
       *
       * 오프사이드를 지키는 방식(`beginRestart`)과 같은 원리이되, 카드는
       * 2.2초를 살아서 다음 반칙까지 남을 수 있다. 그래서 시각이 아니라
       * **같은 틱의 반칙인가**로 따진다.
       */
      const carded = this.cardTick === e.tick ? this.whistle : null
      // 반칙은 우리 선수가 범한다. 차는 쪽은 상대다
      if (at) this.beginRestart('FREE_KICK', 'AWAY', clamp(at.x, 2, PITCH_W - 2), clamp(at.y, 2, PITCH_H - 2))
      if (carded?.kind === 'CARD') this.whistle = carded
    }
    this.lastLogLen = state.log.length
  }
  ...
  private updateRestart(state: MatchState, step: number) {
    ...
    for (const p of this.players) {
      ...
      if (reshape) {
         ...
         * ★ **`settled` 를 건드리지 않는다.**
         *
         * 전에는 여기서 매 프레임 `settled = false` 로 밀어붙였다. 그러면
         * `movePlayer` 의 「도착하면 선다」 판정이 통째로 무력화되어 **자리를
         * 잡고 서는 단계가 사라진다.** QA 가 44만 프레임을 재서 짚었다 —
         * 재개 **직전**에 움직이는 선수가 40.4% 에서 **65.6%** 로 늘었고
         * 평균 속도도 1.52 에서 2.13 m/s 로 올랐다. 프리킥을 차려는 순간
         * 스물두 명 중 열넷이 아직 뛰고 있었다는 뜻이다.
         *
         * 목표를 자기 자리로 바꾸는 것만으로 충분하다 — 멀면 `d > go` 라
         * 저절로 풀려 걸어가고, 도착하면 `d < stop` 이라 저절로 선다.
         * 실제 축구의 데드볼이 그 모양이다: 돌아가서, 서 있는다.
         */
      }
      this.movePlayer(p, step)
    }
  ...
  private updatePressure(holder: VPlayer, dt: number) {
    ...
    if (
      ...
      // ★ 난수를 뽑은 **뒤에** 접촉을 확인한다. 순서를 바꾸면 뽑는 자리가
      // 달라져 같은 시드의 화면이 통째로 바뀐다. 이유는 `FOUL_WATCH` 참조
      nearest < FOUL_CONTACT &&
  ...
  private stepBall(dt: number) {
    ...
    if (b.z <= 0.001) {
      ...
      if (sp > 0) {
        // 구름 저항(일정) + 공기 저항(속도의 제곱). 위 상수 주석을 읽어라
        const next = Math.max(0, sp - (ROLL_DECEL + ROLL_DRAG * sp * sp) * dt)
        b.vx = (b.vx / sp) * next
        b.vy = (b.vy / sp) * next
      }
    }
