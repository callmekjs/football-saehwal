 ...
 *
 * 다섯은 **서로 다른 채널**을 쓴다. 같은 것을 다섯 개 만드는 것보다 다른
 * 것을 다섯 개 만드는 것이 실제로 판을 넓힌다. 계수는 `constants.ts` 의
 * `ORDERS` 에 있고, 어느 국면에서 이로운지는 거기 실측과 함께 적어뒀다.
 * - `HOLD` — 상대 슛이 골이 될 확률을 깎는다. 지키는 판에서 이롭다
 * - `DROP_BACK` — 상대가 오는 횟수와 넣는 확률을 함께 깎는다. 지키는 판
 * - `PUSH_UP` — 우리 전진 시도를 늘린다. 쫓는 판에서 이롭다
 * - `BACK_OFF` — **경고를 안고 있을 때만** 이롭다. 반칙·퇴장 후보에서도
 *   빠지지만 그건 값의 본체가 아니다 — 이 엔진에서 한 명을 잃는 대가가
 *   거의 없어서, 퇴장을 41%에서 14%로 줄여도 통과율이 안 움직였다
 * - `CONSERVE` — **지친 선수일수록** 이롭다. 체력이 가득이면 이득이 0이다
 *
 * `DROP_BACK` · `PUSH_UP` 은 위 계수에 더해 **자리 자체를 바꾼다** — 실점은
 * 수비 자원 중 가장 느린 하나로, 득점은 공격 자원 중 마무리가 가장 좋은
 * 하나로 정해지므로, 누가 어느 줄에 서 있느냐가 곧 확률이다
 */
...
export interface PlayerState {
   ...
   * 지금 포메이션에서 맡은 기본 역할.
   *
   * 등록 포지션과 포메이션의 자리 수가 다를 때 배치판이 정한 역할을
   * 경기 계산도 그대로 읽는다. 자유 좌표와 줄 이동 지시는 이 값보다
   * 우선한다. 예전 저장 상태처럼 값이 없으면 등록 포지션으로 돌아간다.
   */
  formationRole?: Position
  /**
